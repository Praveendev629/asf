...
export default function ProductCard({ product }: { product: ProductCardData }) {
  ...
  return (
    <motion.div whileHover={{ y: -4 }} className="card p-3 flex flex-col group relative overflow-hidden">
      {discount > 0 && <span className="absolute top-3 left-3 z-10 bg-asf-copper text-white text-[11px] font-bold px-2 py-1 rounded-lg">{discount}% OFF</span>}
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <button disabled={outOfStock} onClick={() => addItem({ productId: product._id, name: product.name, image: product.images[0] || "", price: product.price, mrp: product.mrp, unit: product.unit, stock: product.stock })} className="bg-asf-slateDeep disabled:opacity-40 text-white rounded-lg w-9 h-9 flex items-center justify-center hover:bg-asf-copper transition">
          <Plus size={18} />
        </button>
      </div>
    </motion.div>
