...
interface Product { _id: string; name: string; slug: string; description: string; images: string[]; category: string; unit: string; mrp: number; price: number; stock: number; rating: number; ratingCount: number; }
...
export default function ProductPage() {
  ...
  useEffect(() => { fetch(`/api/products/${params.id}`).then((res) => res.json()).then((data) => setProduct(data.product)).finally(() => setLoading(false)); }, [params.id]);
  ...
  function handleBuyNow() {
    if (!product) return;
    addItem({ productId: product._id, name: product.name, image: product.images[0] || "", price: product.price, mrp: product.mrp, unit: product.unit, stock: product.stock }, qty);
    ...
    <div className="container-app py-10 grid md:grid-cols-2 gap-10">
      <div>
        <div className="relative aspect-square rounded-2xl overflow-hidden bg-asf-mist mb-4">
          <Image src={product.images[activeImage] || "https://images.unsplash.com/photo-1542838132-92c53300491e?w=600"} alt={product.name} fill className="object-cover" />
        </div>
        {product.images.length > 1 && (
          <div className="flex gap-3">
            {product.images.map((img, i) => (
              <button key={i} onClick={() => setActiveImage(i)} className={`relative w-16 h-16 rounded-xl overflow-hidden border-2 ${activeImage === i ? "border-asf-copper" : "border-transparent"}`}>
      ...
      <div>
        ...
        <div className="flex items-baseline gap-3 mb-1">
          ...
          {product.mrp > product.price && <><span className="text-lg strike text-asf-slate">₹{product.mrp}</span><span className="text-asf-copper font-semibold text-sm">{discount}% OFF</span></>}
        </div>
        {product.mrp > product.price && <p className="text-sm text-green-700 mb-6">You save ₹{product.mrp - product.price}</p>}
