      ...
      <div className="container-app flex items-center gap-4 py-3">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <div className="relative w-9 h-9">
            <Image src="/logo.png" alt="ASF" fill className="object-contain" />
          </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          {isAdmin && (
            <Link
              href="/admin"
              className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-asf-slateDeep hover:text-asf-copper transition"
            >
          ...
          <Link href="/cart" className="relative">
            ...
            {count > 0 && (
              <span className="absolute -top-2 -right-2 bg-asf-copper text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
                {count}
              </span>
            )}
          </Link>
          ...
          {firebaseUser ? (
            <div className="flex items-center gap-2">
              {profile?.photoURL ? (
                <div className="relative w-8 h-8 rounded-full overflow-hidden border border-asf-mist">
                  <Image src={profile.photoURL} alt={profile.name} fill className="object-cover" />
                </div>
              ) : (
                <User size={20} />
              )}
              <button onClick={() => logout()} title="Sign out" className="text-asf-slate hover:text-asf-copper">
                <LogOut size={18} />
              </button>
            </div>
          ) : (
