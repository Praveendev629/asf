...
export default function AdminPage() {
  ...
  async function loadAll() {
    await Promise.all([loadProducts(), loadOrders(), loadUsers(), loadPartners()]);
  }
  ...
  async function handlePhoneUpdate(orderId: string, phone: string) {
    await fetch(`/api/admin/orders/${orderId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleTogglePartner(partnerId: string, isAvailable: boolean) {
    await fetch("/api/admin/delivery-partners", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: partnerId, isAvailable }),
    });
    loadPartners();
  }
  ...
  return (
    <div className="container-app py-10">
      <h1 className="font-display text-2xl font-semibold text-asf-slateDeep mb-6">
        ASF Admin Dashboard
      </h1>
      ...
      <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
        {tabs.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 whitespace-nowrap transition ${
              tab === t.key
                ? "bg-asf-slateDeep text-white"
                : "bg-white border border-asf-mist text-asf-slate hover:border-asf-copper"
            }`}
          >
            <t.icon size={16} />
            {t.label}
            ...
            <div key={o._id} className="card p-4">
                ...
                <div>
                  ...
                  {o.deliveryPartner && (
                    <p className="text-xs text-green-600 flex items-center gap-1 mt-1">
                      <Truck size={12} /> {o.deliveryPartner.name} · +91 {o.deliveryPartner.phone}
                    </p>
                  )}
                </div>
              ...
              {expandedOrder === o._id && (
                <div className="mt-4 pt-4 border-t border-asf-mist space-y-4">
                  {/* Address with Map */}
                  <div>
                    <p className="text-sm font-medium text-asf-slateDeep mb-1 flex items-center gap-1">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        defaultValue={o.userPhone}
                        onBlur={(e) => handlePhoneUpdate(o._id, e.target.value)}
                        className="border border-asf-mist rounded-xl px-3 py-2 text-sm flex-1"
                  ...
                  <div>
                    ...
                    <select
                      value={partners.find((p) => p.name === o.deliveryPartner?.name)?._id || ""}
                      onChange={(e) => {
                        if (e.target.value) handleAssignPartner(o._id, e.target.value);
                      }}
                      className="border border-asf-mist rounded-xl px-3 py-2 text-sm w-full"
                    >
            ...
            <div key={u._id} className="card p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2 flex-1">
                  <div className="flex items-center gap-2 text-sm text-asf-slateDeep">
                    <Users size={14} className="text-asf-slate" />
                    <span className="font-medium">{u.name}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-asf-slate">
                    <Mail size={14} />
                    {u.email}
                  </div>
                  {u.phone && (
                  ...
                  {u.address && (
                    <div className="flex items-start gap-2 text-sm text-asf-slate">
                      <MapPin size={14} className="mt-0.5 shrink-0" />
                      <span>
                        {u.address.line1}
                        {u.address.line2 ? `, ${u.address.line2}` : ""}, {u.address.city},{" "}
                        {u.address.state} - {u.address.pincode}
                      </span>
                    </div>
                  )}
                </div>
                {u.address?.lat && u.address?.lng && (
                  <a
                    href={`https://www.google.com/maps?q=${u.address.lat},${u.address.lng}`}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <p className="text-xs text-asf-slate mt-2">
      ...
      {tab === "partners" && (
        <div className="space-y-3">
          {partners.map((p) => (
            <div key={p._id} className="card p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2 flex-1">
                  <div className="flex items-center gap-2 text-sm text-asf-slateDeep">
                    <Truck size={14} className="text-asf-slate" />
                ...
                <button
                  onClick={() => handleTogglePartner(p._id, !p.isAvailable)}
              ...
              {p.currentLocation && (
                <a
                  href={`https://www.google.com/maps?q=${p.currentLocation.lat},${p.currentLocation.lng}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-blue-600 hover:underline mt-2 inline-flex items-center gap-1"
                >
                  <MapPin size={12} /> View current location
                </a>
              )}
              <p className="text-xs text-asf-slate mt-2">
          ...
          {partners.length === 0 && (
            <p className="text-asf-slate text-sm">No delivery partners registered yet.</p>
          )}
        </div>
      )}
    </div>
