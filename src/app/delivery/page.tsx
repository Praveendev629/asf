...
export default function DeliveryDashboard() {
  ...
  async function handleUpdateStatus(orderId: string, status: string) {
    ...
    // Include current location for ETA update
    if (navigator.geolocation) {
      try {
      ...
      <header className="bg-gray-900 text-white px-4 py-4 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Truck size={24} />
            <div>
              <h1 className="font-semibold text-lg">ASF Delivery</h1>
              {partner && <p className="text-white/60 text-xs">{partner.name}</p>}
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-medium">
              {activeTab === "available" ? "No available orders" : "No orders assigned yet"}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {activeTab === "available" ? (
                    <button
                      ...
                      className="flex-1 bg-gray-900 text-white py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:bg-gray-800 transition"
                    >
                    ...
                    <>
                      {order.deliveryAddress.lat && order.deliveryAddress.lng && (
                        <button
                          ...
                          className="flex-1 bg-blue-600 text-white py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:bg-blue-700 transition"
                        >
                          <Navigation size={16} />
                          Navigate
                        </button>
