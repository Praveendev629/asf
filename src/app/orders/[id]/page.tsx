  ...
  return (
    <div className="container-app py-10 max-w-2xl">
                ...
                <div className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      done ? "bg-asf-copper text-white" : "bg-asf-mist text-asf-slate"
                    }`}
                  >
                    <Icon size={18} />
                  </div>
                  {i < STAGE_ORDER.length - 1 && (
                    <div className={`w-0.5 flex-1 min-h-[24px] ${i < currentIndex ? "bg-asf-copper" : "bg-asf-mist"}`} />
                  )}
                </div>
                <div className="pb-6">
                  <p className={`font-medium ${done ? "text-asf-slateDeep" : "text-asf-slate"}`}>{meta.label}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="card p-6 mb-6">
        ...
        <div className="border-t border-asf-mist mt-4 pt-3 flex justify-between font-semibold">
          <span>Total</span>
