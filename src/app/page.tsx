      ...
      <section className="bg-gradient-to-br from-asf-slateDeep to-asf-slateDark text-white py-16">
        <div className="container-app">
          <motion.h1
            ...
            className="font-display text-3xl sm:text-5xl font-semibold max-w-2xl leading-tight"
          >
            Fresh groceries, delivered with premium care.
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.15 }}
            className="text-asf-cream/70 mt-4 max-w-lg"
          >
            Curated produce, real-time order tracking, and a shopping experience designed around you.
          </motion.p>
        </div>
      </section>
          ...
          {CATEGORIES.map((c) => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium border transition ${
                category === c
                  ? "bg-asf-slateDeep text-white border-asf-slateDeep"
                  : "bg-white text-asf-slate border-asf-mist hover:border-asf-copper"
              }`}
            >
              {c === "all" ? "All" : c}
            </button>
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="card p-3 h-64 animate-pulse bg-asf-mist/50" />
            ))}
          </div>
        ) : products.length === 0 ? (
