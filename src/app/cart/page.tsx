  ...
  if (items.length === 0) {
    return (
      <div className="container-app py-20 text-center">
        ...
        <p className="font-display text-2xl text-asf-slateDeep mb-2">Your cart is empty</p>
        <Link href="/" className="text-asf-copper font-medium">
          Continue shopping
        </Link>
      </div>
    );
    ...
    <div className="container-app py-10 grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-4">
        ...
        {items.map((item) => (
          <div key={item.productId} className="card p-4 flex gap-4 items-center">
            <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-asf-mist shrink-0">
              <Image
            ...
            <div className="flex-1">
              <p className="font-medium text-asf-slateDeep">{item.name}</p>
              <p className="text-xs text-asf-slate mb-2">{item.unit}</p>
              <p className="font-semibold text-asf-slateDeep">₹{item.price}</p>
            </div>
            ...
            <button onClick={() => removeItem(item.productId)} className="text-asf-slate hover:text-red-500">
              <Trash2 size={18} />
            </button>
          </div>
        ))}
      ...
      <div className="card p-6 h-fit sticky top-24">
        ...
        <div className="space-y-2 text-sm mb-4">
          ...
          <div className="flex justify-between">
            <span className="text-asf-slate">Delivery fee</span>
            <span>{deliveryFee === 0 ? "Free" : `₹${deliveryFee}`}</span>
          </div>
        </div>
        <div className="flex justify-between font-semibold text-lg border-t border-asf-mist pt-4 mb-6">
          <span>Total</span>
